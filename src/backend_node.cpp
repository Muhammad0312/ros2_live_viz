...
public:
    LiveVizBackendNode(
        ...
        : Node("live_viz_backend"),
          ws_server_(cli.port, web_root),
          ppid_(cli.ppid),
          intent_tree_json_(load_intent_tree(cli.intent_file))
    {
        ...
        ws_server_.set_message_handler(
            std::bind(&LiveVizBackendNode::on_ws_message,
                      this, std::placeholders::_1));
        ws_server_.start();

        const auto actual_port = ws_server_.actual_port();
        RCLCPP_INFO(this->get_logger(),
            "live_viz_backend started | port=%u | ppid=%d",
            actual_port, static_cast<int>(ppid_));
        ...
        graph_timer_ = this->create_wall_timer(
            500ms,
            std::bind(&LiveVizBackendNode::poll_graph, this));

        // ── Hz metrics push at 1 Hz ──
        hz_timer_ = this->create_wall_timer(
            1000ms,
            std::bind(&LiveVizBackendNode::push_hz_metrics, this));
...
private:
    ...
    static constexpr int FORCE_FULL_SEND_INTERVAL = 10; // Every 10 ticks = 5s at 2Hz
    ...
    void poll_graph()
    {
        ...
        RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 5000,
            "[poll] Discovered %zu nodes", names_namespaces.size());

        if (names_namespaces.empty()) return;
        ...
        for (const auto & nn : names_namespaces) {
            ...
            // Canonical full name
            std::string fqn = node_ns +
                (node_ns.back() == '/' ? "" : "/") + node_name;
            if (fqn == "/") fqn = "root";
            current.nodes.insert(fqn);

            // Publishers
            auto pubs = this->get_node_graph_interface()
            ...
            // Subscribers
            auto subs = this->get_node_graph_interface()
        ...
        const bool is_initial = added_nodes.size() == current.nodes.size() &&
                                 removed_nodes.empty();
        ...
        const bool force_resend = (force_full_send_counter_ >= FORCE_FULL_SEND_INTERVAL);
        if (force_resend) force_full_send_counter_ = 0;

        if (!has_changes && !is_initial && !force_resend) return;
    ...
    void update_subscriptions(const std::vector<std::string> & targets)
    {
        ...
        for (auto it = monitored_topics_.begin();
             it != monitored_topics_.end();) {
            if (std::find(targets.begin(), targets.end(),
                          it->first) == targets.end()) {
                RCLCPP_INFO(this->get_logger(),
                    "[Hz] Unsubscribing: %s", it->first.c_str());
        ...
        for (const auto & t : targets) {
            if (monitored_topics_.count(t)) continue;
            ...
            if (it != topic_names_and_types.end() && !it->second.empty()) {
                ...
                try {
                    ...
                    RCLCPP_INFO(this->get_logger(),
                        "[Hz] Monitoring: %s  type: %s",
                        t.c_str(), it->second[0].c_str());
                } catch (const std::exception & e) {
                    RCLCPP_ERROR(this->get_logger(),
                        "[Hz] Failed to subscribe: %s — %s",
                        t.c_str(), e.what());
                }
            }
        }
    ...
    {
        std::lock_guard<std::mutex> lock(metrics_mutex_);
        if (monitored_topics_.empty()) return;
...
int main(int argc, char * argv[])
{
    ...
    std::string web_root;
    try {
        web_root = ament_index_cpp::get_package_share_directory("ros2_live_viz") + "/web";
    } catch (const std::exception & e) {
    ...
    RCLCPP_INFO(node->get_logger(),
        "MultiThreadedExecutor spinning. Web root: %s",
        web_root.c_str());
